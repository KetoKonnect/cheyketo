...
export default {
    ...
    beforeMount() {
        axios
            .get(process.env.MIX_ADMIN_URL + "/api/orders/" + this.id)
            .then(response => {
    ...
    methods: {
        getProductsThumbnails(product_id, index) {
            axios
                .get(process.env.MIX_ADMIN_URL + "/api/products/" + product_id)
                .then(response => {
        ...
        getLineItemImage(index) {
            let result = process.env.MIX_APP_URL + "/";
        ...
        updateOrderStatus(status) {
            Swal.fire({
                title: "Warning!",
                text:
                    "Please confirm change of status, customer will be notified",
                icon: "warning",
